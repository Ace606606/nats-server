...
our %config = (
  ...
  openssldir => "/home/gryakin/.local/openssl",
  options => "--prefix=/home/gryakin/.local --openssldir=/home/gryakin/.local/openssl enable-shared enable-zlib no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-tests no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.30.0",
  perlargv => [ "linux-x86_64", "--prefix=/home/gryakin/.local", "--openssldir=/home/gryakin/.local/openssl", "no-tests", "shared", "zlib" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
